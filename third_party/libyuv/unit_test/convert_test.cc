...
TEST_F(LibYUVConvertTest, CropNV12) {
  ...
  ConvertToI420(src_y, sample_size,
                ...
                dii_libyuv::kRotate0, dii_libyuv::FOURCC_NV12);
...
TEST_F(LibYUVConvertTest, NAME) {                                              \
  ...
  dii_libyuv::UYVYTOI420(orig_uyvy, 4 * SUBSAMPLE(kWidth, 2),                      \
                     orig_y, kWidth,                                           \
                     orig_u, SUBSAMPLE(kWidth, 2),                             \
                     orig_v, SUBSAMPLE(kWidth, 2),                             \
                     kWidth, kHeight);                                         \
  dii_libyuv::I420ToNV12(orig_y, kWidth,                                           \
                     orig_u, SUBSAMPLE(kWidth, 2),                             \
                     orig_v, SUBSAMPLE(kWidth, 2),                             \
                     dst_y_orig, kWidth,                                       \
  ...
  /* Convert to NV12 */                                                        \
  for (int i = 0; i < benchmark_iterations_; ++i) {                            \
    dii_libyuv::UYVYTONV12(orig_uyvy, 4 * SUBSAMPLE(kWidth, 2),                    \
                       dst_y, kWidth,                                          \
