...
static int TileARGBScale(const uint8* src_argb, int src_stride_argb,
                         ...
                         dii_libyuv::FilterMode filtering) {
  for (int y = 0; y < dst_height; y += kTileY) {
    for (int x = 0; x < dst_width; x += kTileX) {
      ...
      int r = dii_libyuv::ARGBScaleClip(src_argb, src_stride_argb,
...
int main(int argc, const char* argv[]) {
  ...
  for (number_of_frames = 0; ; ++number_of_frames) {
    ...
    if (org_is_argb && attenuate) {
      dii_libyuv::ARGBAttenuate(ch_org, 0, ch_org, 0, org_size / 4, 1);
    }
    // ARGB unattenuate frame
    if (org_is_argb && unattenuate) {
      dii_libyuv::ARGBUnattenuate(ch_org, 0, ch_org, 0, org_size / 4, 1);
    }
    ...
    for (int cur_rec = 0; cur_rec < num_rec; ++cur_rec) {
      ...
      if (org_is_yuv) {
        ...
        I420Scale(ch_org, src_width,
                  ...
                  dst_width, dst_height,
                      static_cast<dii_libyuv::FilterMode>(filter));
      } else {
      ...
      if (!org_is_yuv && rec_is_yuv) {
        ...
        dii_libyuv::ARGBToI420(ch_dst, dst_width * 4,
                           ch_rec, dst_width,
