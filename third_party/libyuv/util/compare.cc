...
int main(int argc, char** argv) {
  ...
  do {
    ...
    if (amt1 > 0) hash1 = dii_libyuv::HashDjb2(buf1, amt1, hash1);
    if (fin2) {
      amt2 = static_cast<int>(fread(buf2, 1, kBlockSize, fin2));
      if (amt2 > 0) hash2 = dii_libyuv::HashDjb2(buf2, amt2, hash2);
      int amt_min = (amt1 < amt2) ? amt1 : amt2;
      size_min += amt_min;
      sum_square_err += dii_libyuv::ComputeSumSquareError(buf1, buf2, amt_min);
    }
  } while (amt1 > 0 || amt2 > 0);
  ...
  if (fin2) {
    ...
    printf(", mse %.2f", mse);
    double psnr = dii_libyuv::SumSquareErrorToPsnr(sum_square_err, size_min);
    printf(", psnr %.2f\n", psnr);
    fclose(fin2);
  }
  fclose(fin1);
