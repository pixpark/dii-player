...
class VideoFrameBuffer : public dii_rtc::RefCountInterface {
 public:
  ...
  // native handle.
  virtual dii_rtc::scoped_refptr<VideoFrameBuffer> NativeToI420Buffer() = 0;
...
class I420Buffer : public VideoFrameBuffer {
 public:
  ...
  static dii_rtc::scoped_refptr<I420Buffer> Create(int width, int height);
  static dii_rtc::scoped_refptr<I420Buffer> Create(int width,
                                               int height,
                                               int stride_y,
                                               int stride_u,
                                               int stride_v);
  ...
  dii_rtc::scoped_refptr<VideoFrameBuffer> NativeToI420Buffer() override;

  // Create a new buffer and copy the pixel data.
  static dii_rtc::scoped_refptr<I420Buffer> Copy(
      const dii_rtc::scoped_refptr<VideoFrameBuffer>& buffer);

  // Scale the cropped area of |src| to the size of |this| buffer, and
  // write the result into |this|.
  void CropAndScaleFrom(const dii_rtc::scoped_refptr<VideoFrameBuffer>& src,
                        int offset_x,
                        int offset_y,
                        int crop_width,
                        int crop_height);
  ...
  void ScaleFrom(const dii_rtc::scoped_refptr<VideoFrameBuffer>& src);

  // Create a new buffer with identical strides, and copy the pixel data.
  static dii_rtc::scoped_refptr<I420Buffer> CopyKeepStride(
      const dii_rtc::scoped_refptr<VideoFrameBuffer>& buffer);
...
class WrappedI420Buffer : public dii_media_kit::VideoFrameBuffer {
 public:
 ...
 private:
  friend class dii_rtc::RefCountedObject<WrappedI420Buffer>;
  ...
  const int u_stride_;
  const int v_stride_;
  dii_rtc::Callback0<void> no_longer_used_cb_;
};
