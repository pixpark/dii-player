...
dii_rtc::scoped_refptr<I420Buffer> CreateGradient(int width, int height) {
  dii_rtc::scoped_refptr<I420Buffer> buffer(
...
void CheckCrop(dii_media_kit::VideoFrameBuffer* frame,
               double offset_x,
               double offset_y,
               double rel_width,
               double rel_height) {
...
TEST(TestVideoFrame, TextureInitialValues) {
  ...
  VideoFrame frame = test::FakeNativeHandle::CreateFrame(
      handle, 640, 480, 100, 10, dii_media_kit::kVideoRotation_0);
...
TEST(TestI420FrameBuffer, CropYCenter) {
  dii_rtc::scoped_refptr<I420Buffer> buf = CreateGradient(100, 200);
...
TEST(TestI420FrameBuffer, CropAndScale16x9) {
  dii_rtc::scoped_refptr<I420Buffer> buf = CreateGradient(640, 480);
