...
const static int32_t MaxFrameWidth = 4096;
...
VideoFrame::VideoFrame(const dii_rtc::scoped_refptr<VideoFrameBuffer>& buffer,
                       uint32_t timestamp,
...
int VideoFrame::CreateFrame(const uint8_t* buffer_y,
    ...
    if(buffer_y == nullptr      ||
       ...
       buffer_v == nullptr      ||
       width > MaxFrameWidth    ||
       height > MaxFrameWidth   ||
       stride_y > MaxFrameWidth ||
       stride_u > MaxFrameWidth ||
       stride_v > MaxFrameWidth) {
        return -1;
    }
...
const dii_rtc::scoped_refptr<VideoFrameBuffer>& VideoFrame::video_frame_buffer()
    const {
  return video_frame_buffer_;
}
