...
void GenerateFakeSps(SpsMode mode, dii_rtc::Buffer* out_buffer) {
  uint8_t rbsp[kSpsBufferMaxSize] = {0};
  dii_rtc::BitBufferWriter writer(rbsp, kSpsBufferMaxSize);
  // Profile byte.
  writer.WriteUInt8(0);
...
void TestSps(SpsMode mode, SpsVuiRewriter::ParseResult expected_parse_result) {
  dii_rtc::LogMessage::LogToDebug(dii_rtc::LS_VERBOSE);
  dii_rtc::Buffer buffer;
