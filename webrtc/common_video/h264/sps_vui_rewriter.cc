...
bool CopyAndRewriteVui(Sps sps,
                       dii_rtc::BitBuffer* source,
                       dii_rtc::BitBufferWriter* destination,
                       SpsVuiRewriter::ParseResult* out_vui_rewritten);
bool CopyHrdParameters(dii_rtc::BitBuffer* source,
                       dii_rtc::BitBufferWriter* destination);
bool AddBitstreamRestriction(dii_rtc::BitBufferWriter* destination,
                             uint32_t max_num_ref_frames);
bool CopyRemainingBits(dii_rtc::BitBuffer* source,
                       dii_rtc::BitBufferWriter* destination);

SpsVuiRewriter::ParseResult SpsVuiRewriter::ParseAndRewriteSps(
    const uint8_t* buffer,
    size_t length,
    dii_rtc::Optional<SpsParser::SpsState>* sps,
    dii_rtc::Buffer* destination) {
  dii_rtc::BitBuffer source_buffer(buffer, length);
  dii_rtc::Optional<SpsParser::SpsState> sps_state =
  ...
  *sps = sps_state;
  ...
  // write with.
  dii_rtc::Buffer out_buffer(length + kMaxVuiSpsIncrease);
  dii_rtc::BitBufferWriter sps_writer(out_buffer.data(), out_buffer.size());
...
// These functions are similar to dii_media_kit::H264SpsParser::Parse, and based on the
