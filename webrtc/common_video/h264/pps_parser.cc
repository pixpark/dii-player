...
#define RETURN_EMPTY_ON_FAIL(x)                  \
  if (!(x)) {                                    \
    return dii_rtc::Optional<PpsParser::PpsState>(); \
  }
...
dii_rtc::Optional<PpsParser::PpsState> PpsParser::ParsePps(const uint8_t* data,
                                                       size_t length) {
  // First, parse out rbsp, which is basically the source buffer minus emulation
  // bytes (the last byte of a 0x00 0x00 0x03 sequence). RBSP is defined in
  // section 7.3.1 of the H.264 standard.
  std::unique_ptr<dii_rtc::Buffer> unpacked_buffer = H264::ParseRbsp(data, length);
  dii_rtc::BitBuffer bit_buffer(unpacked_buffer->data(), unpacked_buffer->size());
  return ParseInternal(&bit_buffer);
}

dii_rtc::Optional<PpsParser::PpsState> PpsParser::ParseInternal(
    dii_rtc::BitBuffer* bit_buffer) {
  PpsState pps;
  ...
  return dii_rtc::Optional<PpsParser::PpsState>(pps);
}
