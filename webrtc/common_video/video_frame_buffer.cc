...
dii_rtc::scoped_refptr<I420Buffer> I420Buffer::Create(int width, int height) {
  return new dii_rtc::RefCountedObject<I420Buffer>(width, height);
}

dii_rtc::scoped_refptr<I420Buffer> I420Buffer::Create(int width,
                                                  int height,
                                                  int stride_y,
                                                  int stride_u,
                                                  int stride_v) {
  return new dii_rtc::RefCountedObject<I420Buffer>(
...
dii_rtc::scoped_refptr<VideoFrameBuffer> I420Buffer::NativeToI420Buffer() {
  RTC_NOTREACHED();
  return nullptr;
}

dii_rtc::scoped_refptr<I420Buffer> I420Buffer::Copy(
    const dii_rtc::scoped_refptr<VideoFrameBuffer>& source) {
  int width = source->width();
  int height = source->height();
  dii_rtc::scoped_refptr<I420Buffer> target = I420Buffer::Create(width, height);
  RTC_CHECK(dii_libyuv::I420Copy(source->DataY(), source->StrideY(),
...
void I420Buffer::SetToBlack() {
  RTC_CHECK(dii_libyuv::I420Rect(MutableDataY(), StrideY(),
...
void I420Buffer::CropAndScaleFrom(
    const dii_rtc::scoped_refptr<VideoFrameBuffer>& src,
    int offset_x,
    int offset_y,
    int crop_width,
    int crop_height) {
  ...
  int res = dii_libyuv::I420Scale(y_plane, src->StrideY(),
                              u_plane, src->StrideU(),
                              v_plane, src->StrideV(),
                              ...
                              MutableDataU(), StrideU(),
                              MutableDataV(), StrideV(),
                              width(), height(), dii_libyuv::kFilterBox);
...
void I420Buffer::ScaleFrom(const dii_rtc::scoped_refptr<VideoFrameBuffer>& src) {
  CropAndScaleFrom(src, 0, 0, src->width(), src->height());
}

dii_rtc::scoped_refptr<I420Buffer> I420Buffer::CopyKeepStride(
    const dii_rtc::scoped_refptr<VideoFrameBuffer>& source) {
  int width = source->width();
  int height = source->height();
  ...
  dii_rtc::scoped_refptr<I420Buffer> target =
      I420Buffer::Create(width, height, stride_y, stride_u, stride_v);
  RTC_CHECK(dii_libyuv::I420Copy(source->DataY(), stride_y,
...
WrappedI420Buffer::WrappedI420Buffer(int width,
                                     ...
                                     const uint8_t* v_plane,
                                     int v_stride,
                                     const dii_rtc::Callback0<void>& no_longer_used)
    : width_(width),
      height_(height),
      y_plane_(y_plane),
      u_plane_(u_plane),
...
dii_rtc::scoped_refptr<VideoFrameBuffer> WrappedI420Buffer::NativeToI420Buffer() {
  RTC_NOTREACHED();
  return nullptr;
}
