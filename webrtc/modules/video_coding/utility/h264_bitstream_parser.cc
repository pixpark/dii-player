...
bool H264BitstreamParser::ParseNonParameterSetNalu(const uint8_t* source,
  ...
  RTC_CHECK(sps_);
  RTC_CHECK(pps_);
  last_slice_qp_delta_ = dii_rtc::Optional<int32_t>();
  std::unique_ptr<dii_rtc::Buffer> slice_rbsp(
      H264::ParseRbsp(source, source_length));
  dii_rtc::BitBuffer slice_reader(slice_rbsp->data() + H264::kNaluTypeSize,
  ...
  last_slice_qp_delta_ = dii_rtc::Optional<int32_t>(last_slice_qp_delta);
  return true;
}
