...
dii_rtc::CriticalSection ffmpeg_init_lock;
...
int LockManagerOperation(void** lock, AVLockOp op)
  ...
  switch (op) {
    case AV_LOCK_CREATE:
      *lock = new dii_rtc::CriticalSection();
      return 0;
    case AV_LOCK_OBTAIN:
      static_cast<dii_rtc::CriticalSection*>(*lock)->Enter();
      return 0;
    case AV_LOCK_RELEASE:
      static_cast<dii_rtc::CriticalSection*>(*lock)->Leave();
      return 0;
    case AV_LOCK_DESTROY:
      delete static_cast<dii_rtc::CriticalSection*>(*lock);
      *lock = nullptr;
      return 0;
  }
...
void InitializeFFmpeg() {
  dii_rtc::CritScope cs(&ffmpeg_init_lock);
...
int32_t H264DecoderImpl::Decode(const EncodedImage& input_image,
  ...
  dii_rtc::scoped_refptr<VideoFrameBuffer> buf = video_frame->video_frame_buffer();
  if (av_frame_->width != buf->width() || av_frame_->height != buf->height()) {
    dii_rtc::scoped_refptr<VideoFrameBuffer> cropped_buf(
        new dii_rtc::RefCountedObject<WrappedI420Buffer>(
            av_frame_->width, av_frame_->height,
            buf->DataY(), buf->StrideY(),
            buf->DataU(), buf->StrideU(),
            buf->DataV(), buf->StrideV(),
            dii_rtc::KeepRefUntilDone(buf)));
