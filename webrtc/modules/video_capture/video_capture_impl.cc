...
dii_rtc::scoped_refptr<VideoCaptureModule> VideoCaptureImpl::Create(
    const int32_t id,
    VideoCaptureExternal*& externalCapture) {
  dii_rtc::scoped_refptr<VideoCaptureImpl> implementation(
      new dii_rtc::RefCountedObject<VideoCaptureImpl>(id));
...
int64_t VideoCaptureImpl::TimeUntilNextProcess()
{
    ...
    return kProcessIntervalMs -
           (dii_rtc::TimeNanos() - _lastProcessTimeNanos) /
           dii_rtc::kNumNanosecsPerMillisec;
}
...
void VideoCaptureImpl::Process()
{
    ...
    const int64_t now_ns = dii_rtc::TimeNanos();
    _lastProcessTimeNanos = dii_rtc::TimeNanos();
...
VideoCaptureImpl::VideoCaptureImpl(const int32_t id)
    : _id(id),
      ...
      _lastProcessTimeNanos(dii_rtc::TimeNanos()),
      _lastFrameRateCallbackTimeNanos(dii_rtc::TimeNanos()),
      ...
      _lastProcessFrameTimeNanos(dii_rtc::TimeNanos()),
    ...
    int64_t captureTime/*=0*/)
{
    ...
    if (frameInfo.codecType == kVideoCodecUnknown)
    {
        ...
        _captureFrame.set_render_time_ms(dii_rtc::TimeMillis());
...
{
  if (_incomingFrameTimesNanos[0] / dii_rtc::kNumNanosecsPerMicrosec == 0)
    {
        // first no shift
    }
    else
    ...
    _incomingFrameTimesNanos[0] = dii_rtc::TimeNanos();
}
