...
struct {
  ...
  {"org/dii/webrtc/voiceengine/BuildInfo", nullptr},
  {"org/dii/webrtc/voiceengine/WebRtcAudioManager", nullptr},
  {"org/dii/webrtc/voiceengine/WebRtcAudioRecord", nullptr},
  {"org/dii/webrtc/voiceengine/WebRtcAudioTrack", nullptr},
};
...
NativeRegistration::~NativeRegistration() {
  ...
  /**
   * bug-fix: 在多个rtmp 拉流的实例的情况下，一个实例停止拉流会调用到 jni_->UnregisterNatives(j_class_)， 会将 jni 注册的 native 方法从java env 中
   * 移除，而另一个实例此时如果仍在拉流，则会找不到需要的 native 方法，造成崩溃。
   */
