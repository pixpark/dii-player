...
void ProcessThreadImpl::Start() {
  ...
  thread_.reset(
      new dii_rtc::PlatformThread(&ProcessThreadImpl::Run, this, thread_name_));
  thread_->Start();
}
...
void ProcessThreadImpl::PostTask(std::unique_ptr<dii_rtc::QueuedTask> task) {
  // Allowed to be called on any thread.
  {
    dii_rtc::CritScope lock(&lock_);
...
bool ProcessThreadImpl::Process() {
  int64_t now = dii_rtc::TimeMillis();
...

  {
    ...
    for (ModuleCallback& m : modules_) {
      ...
      if (m.next_callback <= now ||
        ...
        int64_t new_now = dii_rtc::TimeMillis();
    ...
    while (!queue_.empty()) {
      dii_rtc::QueuedTask* task = queue_.front();
      queue_.pop();
      lock_.Leave();
      task->Run();
      delete task;
  ...
  int64_t time_to_wait = next_checkpoint - dii_rtc::TimeMillis();
  if (time_to_wait > 0)
