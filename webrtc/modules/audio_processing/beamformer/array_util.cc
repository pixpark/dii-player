...
dii_rtc::Optional<Point> GetDirectionIfLinear(
  ...
  for (size_t i = 2u; i < array_geometry.size(); ++i) {
    ...
    if (!AreParallel(first_pair_direction, pair_direction)) {
      return dii_rtc::Optional<Point>();
    }
  }
  return dii_rtc::Optional<Point>(first_pair_direction);
}

dii_rtc::Optional<Point> GetNormalIfPlanar(
    const std::vector<Point>& array_geometry) {
  RTC_DCHECK_GT(array_geometry.size(), 1);
  const Point first_pair_direction =
      PairDirection(array_geometry[0], array_geometry[1]);
  ...
  return dii_rtc::Optional<Point>(normal_direction);
}

dii_rtc::Optional<Point> GetArrayNormalIfExists(
    const std::vector<Point>& array_geometry) {
  const dii_rtc::Optional<Point> direction = GetDirectionIfLinear(array_geometry);
  if (direction) {
    return dii_rtc::Optional<Point>(Point(direction->y(), -direction->x(), 0.f));
  }
  const dii_rtc::Optional<Point> normal = GetNormalIfPlanar(array_geometry);
