...
AudioDeviceBuffer::AudioDeviceBuffer() :
    ...
    LOG(LS_VERBOSE) << __FUNCTION__ <<  " created";
...
AudioDeviceBuffer::~AudioDeviceBuffer()
{
    LOG(LS_VERBOSE) << __FUNCTION__ << " destroyed";
    {
...
{
    LOG(LS_VERBOSE) << "AudioDeviceBuffer::SetId id=" <<  id << "func: " << __FUNCTION__;
    _id = id;
}
...
int32_t AudioDeviceBuffer::DeliverRecordedData()
{
    ...
    {
        LOG(LS_WARNING)  <<  "failed to deliver recorded data (AudioTransport does not exist)";
        return 0;
    }
...
int32_t AudioDeviceBuffer::RequestPlayoutData(size_t nSamples, int32_t delayMs)
{
    ...
    size_t playChannels = 0;
    {
        ...
        {
            LOG(LS_WARNING) << "invalid number of samples to be played out :" << nSamples;
            return -1;
        }
    }
    ...
    if (_ptrCbAudioTransport == NULL)
    {
        LOG(LS_WARNING) <<"failed to feed data to playout (AudioTransport does not exist)";
        return 0;
    }
...
int32_t AudioDeviceBuffer::GetPlayoutData(void* audioBuffer)
{
    ...
    if (_playSize > kMaxBufferSizeBytes)
    {
      LOG(LS_VERBOSE) << "_playSize " << _playSize <<   " exceeds kMaxBufferSizeBytes in "
