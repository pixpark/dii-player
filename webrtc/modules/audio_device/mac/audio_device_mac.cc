...
int32_t AudioDeviceMac::SetSpeakerVolume(uint32_t volume) {
    ...
    _volumeRatio = (Float32)(volume / 255.0);
    return 0;
}
...
//    return 0;
    ...
    volume = (int32_t)(_volumeRatio * 255);
    return 0;
}
...
int32_t AudioDeviceMac::PlayoutDeviceName(uint16_t index,
  ...
  AudioDeviceID devid = -1;
  int32_t ret = GetDeviceName(kAudioDevicePropertyScopeOutput, index, name, devid);
  snprintf(guid, kAdmMaxGuidSize-1, "%d", devid);
...
int32_t AudioDeviceMac::RecordingDeviceName(uint16_t index,
  ...
  AudioDeviceID devid = -1;
  int32_t ret = GetDeviceName(kAudioDevicePropertyScopeInput, index, name, devid);
  snprintf(guid, kAdmMaxGuidSize-1, "%d", devid);
  guid[kAdmMaxGuidSize-1] = '\0';
  return ret;
}
...
int32_t AudioDeviceMac::StartRecording() {
  ...
  capture_worker_thread_.reset(
      new dii_rtc::PlatformThread(RunCapture, this, "CaptureWorkerThread"));
  RTC_DCHECK(capture_worker_thread_.get());
  capture_worker_thread_->Start();
  capture_worker_thread_->SetPriority(dii_rtc::kRealtimePriority);
...
int32_t AudioDeviceMac::StartPlayout() {
  ...
  render_worker_thread_.reset(
      new dii_rtc::PlatformThread(RunRender, this, "RenderWorkerThread"));
  render_worker_thread_->Start();
  render_worker_thread_->SetPriority(dii_rtc::kRealtimePriority);
...
int32_t AudioDeviceMac::StopPlayout() {
  ...
  // Setting this signal will allow the worker thread to be stopped.
  AtomicSet32(&_renderDeviceIsAlive, 0);  if (render_worker_thread_.get()) {
    _critSect.Leave();
  ...
  if (err != noErr) {
    ...
    for (UInt32 i = 0; i < numberDevices; i++) {
      ...
      if (bufferList->mNumberBuffers > 0) {
        ...
        if (i != 0 && deviceIds[i] == scopedDeviceIds[0] ) { //Delete duplitcation
          continue;
        }
...
bool AudioDeviceMac::RenderWorkerThread() {
  ...
  if (_macBookProPanRight && (_playChannels == 2)) {
    ...
    for (uint32_t sampleIdx = 0; sampleIdx < nOutSamples; sampleIdx += 2) {
      ...
      sampleInt32 = sampleInt32 * _volumeRatio;
      ...
      {
          pPlayBuffer[sampleIdx] = pPlayBuffer[sampleIdx] * _volumeRatio;
      }
  }
