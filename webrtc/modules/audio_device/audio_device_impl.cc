...
dii_rtc::scoped_refptr<AudioDeviceModule> AudioDeviceModule::Create(
    const int32_t id,
    const AudioLayer audio_layer) {
  LOG(INFO) << __FUNCTION__;
  // Create the generic ref counted (platform independent) implementation.
  dii_rtc::scoped_refptr<AudioDeviceModuleImpl> audioDevice(
      new dii_rtc::RefCountedObject<AudioDeviceModuleImpl>(id, audio_layer));
...
AudioDeviceModuleImpl::AudioDeviceModuleImpl(const int32_t id,
    ...
    : _critSect(*CriticalSectionWrapper::CreateCriticalSection()),
      ...
      _lastProcessTime(dii_rtc::TimeMillis()),
