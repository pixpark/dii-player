...
int32_t AudioDeviceWindowsWave::Init()
{
    ...
    const uint32_t nowTime(dii_rtc::TimeMillis());
    ...
    _ptrThread.reset(new dii_rtc::PlatformThread(ThreadFunc, this, threadName));
    _ptrThread->Start();
    _ptrThread->SetPriority(dii_rtc::kRealtimePriority);
...
int32_t AudioDeviceWindowsWave::Terminate()
{
    ...
    if (_ptrThread)
    {
        dii_rtc::PlatformThread* tmpThread = _ptrThread.release();
        _critSect.Leave();
...
int AudioDeviceWindowsWave::PlayProc(LONGLONG& consumedTime)
{
    ...
    if (remTimeMS < thresholdMS + 9)
    {
        ...
        //
        UnLock();
        uint32_t nSamples = _ptrAudioBuffer->RequestPlayoutData(PLAY_BUF_SIZE_IN_SAMPLES, remTimeMS);
        Lock();
