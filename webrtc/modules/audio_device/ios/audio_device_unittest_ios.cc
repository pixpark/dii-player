...
class AudioDeviceTest : public ::testing::Test {
 protected:
  AudioDeviceTest() : test_is_done_(EventWrapper::Create()) {
    old_sev_ = dii_rtc::LogMessage::GetLogToDebug();
    // Set suitable logging level here. Change to dii_rtc::LS_INFO for more verbose
    // output. See webrtc/base/logging.h for complete list of options.
    dii_rtc::LogMessage::LogToDebug(dii_rtc::LS_INFO);
    // Add extra logging fields here (timestamps and thread id).
    // dii_rtc::LogMessage::LogTimestamps();
    dii_rtc::LogMessage::LogThreads();
  ...
  virtual ~AudioDeviceTest() {
    ...
    dii_rtc::LogMessage::LogToDebug(old_sev_);
  }
  ...
  dii_rtc::scoped_refptr<AudioDeviceModule> audio_device() const {
  ...
  dii_rtc::scoped_refptr<AudioDeviceModule> CreateAudioDevice(
      AudioDeviceModule::AudioLayer audio_layer) {
    dii_rtc::scoped_refptr<AudioDeviceModule> module(
  ...
  std::string GetFileName(int sample_rate) {
    ...
    snprintf(fname, sizeof(fname), "audio_device/audio_short%d",
             sample_rate / 1000);
    std::string file_name(dii_media_kit::test::ResourcePath(fname, "pcm"));
  ...
  dii_rtc::scoped_refptr<AudioDeviceModule> audio_device_;
  AudioParameters playout_parameters_;
  AudioParameters record_parameters_;
  dii_rtc::LoggingSeverity old_sev_;
};
...
TEST_F(AudioDeviceTest, StartPlayoutOnTwoInstances) {
  ...
  dii_rtc::scoped_refptr<AudioDeviceModule> second_audio_device =
