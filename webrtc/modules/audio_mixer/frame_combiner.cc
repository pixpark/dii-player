...
void CombineMultipleFrames(
    const std::vector<dii_rtc::ArrayView<const int16_t>>& input_frames,
    bool use_limiter,
    AudioProcessing* limiter,
    AudioFrame* audio_frame_for_mixing) {
  ...
  if (use_limiter) {
    ...
    std::transform(add_buffer.begin(), add_buffer.begin() + frame_length,
                   audio_frame_for_mixing->data_, [](int32_t a) {
                     return dii_rtc::saturated_cast<int16_t>(a / 2);
                   });
    ...
    std::transform(add_buffer.begin(), add_buffer.begin() + frame_length,
                   audio_frame_for_mixing->data_,
                   [](int32_t a) { return dii_rtc::saturated_cast<int16_t>(a); });
  }
}
...
void FrameCombiner::Combine(const std::vector<AudioFrame*>& mix_list,
  ...
  const size_t samples_per_channel = static_cast<size_t>(
      (sample_rate * AudioMixerImpl::kFrameDurationInMs) / 1000);
  ...
  if (mix_list.empty()) {
    ...
    std::vector<dii_rtc::ArrayView<const int16_t>> input_frames;
    for (size_t i = 0; i < mix_list.size(); ++i) {
      input_frames.push_back(dii_rtc::ArrayView<const int16_t>(
