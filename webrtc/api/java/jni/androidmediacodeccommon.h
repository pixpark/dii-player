...
static inline void AllowBlockingCalls() {
  dii_rtc::Thread* current_thread = dii_rtc::Thread::Current();
...
static inline jobject JavaEnumFromIndexAndClassName(
  ...
  const std::string state_class = "org/dii/webrtc/" + state_class_fragment;
...
static inline bool CheckException(JNIEnv* jni) {
  if (jni->ExceptionCheck()) {
    LOG_TAG(dii_rtc::LS_ERROR, TAG_COMMON) << "Java JNI exception.";
