...
class AndroidVideoCapturerJni : public dii_media_kit::AndroidVideoCapturerDelegate {
 public:
  ...
  void Start(int width, int height, int framerate,
             dii_media_kit::AndroidVideoCapturer* capturer) override;
  void Stop() override;
 ...
 private:
  ...
  dii_media_kit::I420BufferPool pre_scale_pool_;
  dii_media_kit::I420BufferPool post_scale_pool_;
  dii_rtc::scoped_refptr<SurfaceTextureHelper> surface_texture_helper_;
  ...
  dii_rtc::CriticalSection capturer_lock_;
  dii_media_kit::AndroidVideoCapturer* capturer_ GUARDED_BY(capturer_lock_);
  // |invoker_| is used to communicate with |capturer_| on the thread Start() is
  // called on.
  std::unique_ptr<dii_rtc::GuardedAsyncInvoker> invoker_ GUARDED_BY(capturer_lock_);
