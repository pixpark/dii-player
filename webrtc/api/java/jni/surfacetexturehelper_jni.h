...
// It is used for creating dii_media_kit::VideoFrameBuffers from a SurfaceTexture when
...
class SurfaceTextureHelper : public dii_rtc::RefCountInterface {
 public:
  // Might return null if creating the Java SurfaceTextureHelper fails.
  static dii_rtc::scoped_refptr<SurfaceTextureHelper> create(
      JNIEnv* jni, const char* thread_name, jobject j_egl_context);

  jobject GetJavaSurfaceTextureHelper() const;

  dii_rtc::scoped_refptr<dii_media_kit::VideoFrameBuffer> CreateTextureFrame(
      int width,
      int height,
