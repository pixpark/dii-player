...
int main(int argc, char** argv) {
  ...
  dii_rtc::FlagList::SetFlagsFromCommandLine(&argc, argv, false);
  if (FLAG_help) {
    dii_rtc::FlagList::Print(NULL, false);
    return 0;
  }

  dii_media_kit::test::SetExecutablePath(argv[0]);
  dii_media_kit::test::InitFieldTrialsFromString(FLAG_force_fieldtrials);
  ...
  dii_rtc::Filesystem::SetOrganizationName("google");
  dii_rtc::Filesystem::SetApplicationName("unittest");

  // By default, log timestamps. Allow overrides by used of a --log flag.
  dii_rtc::LogMessage::LogTimestamps();
  if (*FLAG_log != '\0') {
    dii_rtc::LogMessage::ConfigureLogging(FLAG_log);
  } else if (dii_rtc::LogMessage::GetLogToDebug() > dii_rtc::LS_INFO) {
  ...
  dii_rtc::LogMessage::ConfigureLogging("");
