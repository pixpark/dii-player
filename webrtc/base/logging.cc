...
bool LogMessage::thread_ = true, LogMessage::timestamp_ = true, LogMessage::log_tag_ = true;
...
LogMessage::LogMessage(const char* file,
                       ...
                       int err,
                       const char* module)
    : severity_(sev), code_(code), tag_(kLibjingle) {

    char trace_message[128] = {0};
#ifndef WEBRTC_WIN
    ...
    sprintf(trace_message, "[%4u-%02u-%02u %02u:%02u:%02u:%03u] ",
...
#endif
    print_stream_ << trace_message;
    
    if(log_tag_) {
        print_stream_ << "[Dii Media] ";
    }
...
LogMessage::LogMessage(const char* file,
    ...
    : LogMessage(file, line, sev, -1 /* stream id*/, 0, 0 /* err */, NULL /* module */) {
  tag_ = tag;
  print_stream_ << tag << ": ";
}
...
LogMessage::~LogMessage() {
  ...
  for (auto& kv : streams_) {
    if (severity_ >= kv.second) {
      kv.first->OnLogMessage(code_, str);
    }
  }
}
...
void LogMessage::LogToConsole(bool log_to_stderr) {
...
void LogMessage::ConfigureLogging(const char* params) {
  LoggingSeverity current_level = LS_NONE;
