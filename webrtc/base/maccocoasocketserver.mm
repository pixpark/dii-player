...
@interface MacCocoaSocketServerHelperRtc : NSObject {
  ...
  // dii_rtc::MacCocoaSocketServer owns this object.
  dii_rtc::MacCocoaSocketServer* socketServer_;  // Weak.
}
@end

@implementation MacCocoaSocketServerHelperRtc
- (id)initWithSocketServer:(dii_rtc::MacCocoaSocketServer*)ss {
  self = [super init];
  if (self) {
    socketServer_ = ss;
  }
...
bool MacCocoaSocketServer::Wait(int cms, bool process_io) {
  dii_rtc::ScopedAutoreleasePool pool;
  if (!process_io && cms == 0) {
    // No op.
    return true;
  }
