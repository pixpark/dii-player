...
// calls up the stack.
#define ASSERT(x) \
    (dii_rtc::AssertNoBreak((x), __FUNCTION__, __FILE__, __LINE__, #x) ? \
     (void)(1) : __debugbreak())
#else
#define ASSERT(x) \
    (void)dii_rtc::Assert((x), __FUNCTION__, __FILE__, __LINE__, #x)
#endif
#endif
...
#else
#define VERIFY(x) dii_rtc::Assert((x), __FUNCTION__, __FILE__, __LINE__, #x)
#endif
#endif
...
#ifndef VERIFY
#define VERIFY(x) dii_rtc::ImplicitCastToBool(x)
#endif
