...
AsyncSocket* AsyncTCPSocketBase::ConnectSocket(
    dii_rtc::AsyncSocket* socket,
    const dii_rtc::SocketAddress& bind_address,
    const dii_rtc::SocketAddress& remote_address) {
  std::unique_ptr<dii_rtc::AsyncSocket> owned_socket(socket);
...
void AsyncTCPSocketBase::OnReadEvent(AsyncSocket* socket) {
  ...
  if (listen_) {
    dii_rtc::SocketAddress address;
    dii_rtc::AsyncSocket* new_socket = socket->Accept(&address);
    if (!new_socket) {
...
int AsyncTCPSocket::Send(const void *pv, size_t cb,
  ...
  dii_rtc::SentPacket sent_packet(options.packet_id, dii_rtc::TimeMillis());
...
void AsyncTCPSocket::ProcessInput(char * data, size_t* len) {
  ...
  while (true) {
    ...
    PacketLength pkt_len = dii_rtc::GetBE16(data);
