...
const char* inet_ntop_v4(const void* src, char* dst, socklen_t size) {
  ...
  dii_rtc::sprintfn(dst, size, "%d.%d.%d.%d",
...
const char* inet_ntop_v6(const void* src, char* dst, socklen_t size) {
  ...
  if (runpos[0] == 1 && (maxpos == 5 ||
    ...
    *cursor++ = ':';
    if (maxpos == 4) {
      cursor += dii_rtc::sprintfn(cursor, INET6_ADDRSTRLEN - 2, "ffff:");
    }
    ...
    for (int i = 0; i < run_array_size; ++i) {
      if (runpos[i] == -1) {
        cursor += dii_rtc::sprintfn(cursor,
...
int inet_pton_v6(const char* src, void* dst) {
  ...
  if (*readcursor == ':' && *(readcursor+1) == ':' &&
    ...
    if (dii_rtc::strchr(addrstart, ".")) {
      const char* colon = dii_rtc::strchr(addrstart, "::");
      if (colon) {
        uint16_t a_short;
        int bytesread = 0;
...
bool Utf8ToWindowsFilename(const std::string& utf8, std::wstring* filename) {
  ...
  std::replace(wfilename.data(), wfilename.data() + wlen, L'/', L'\\');
  // Convert to complete filename
  DWORD full_len = ::GetFullPathName(wfilename.data(), 0, NULL, NULL);
  if (0 == full_len) {
    return false;
  }
  wchar_t* filepart = NULL;
  std::vector<wchar_t> full_filename(full_len + 6);
  wchar_t* start = full_filename.data() + 6;
  if (0 == ::GetFullPathName(wfilename.data(), full_len, start, &filepart)) {
    return false;
  }
  ...
  if ((start[0] != L'\\') || (start[1] != L'\\')) {
    ...
    //      Becomes: \\?\<pathname>
    start -= 4;
    ASSERT(start >= full_filename.data());
