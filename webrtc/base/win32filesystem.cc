...
bool Win32Filesystem::CreatePrivateFile(const Pathname &filename) {
  ...
  if (!::OpenProcessToken(::GetCurrentProcess(),
                          TOKEN_QUERY,
                          &process_token)) {
    LOG(LS_ERROR) << "OpenProcessToken() failed";
    return false;
  }
  ...
  if (!success) {
    LOG(LS_ERROR) << "GetTokenInformation() failed";
    return false;
  }

  if (!IsValidSid(token_user->User.Sid)) {
    LOG(LS_ERROR) << "Current process has invalid user SID";
    return false;
  }
  ...
  if (!::InitializeAcl(acl, acl_size, ACL_REVISION)) {
    LOG(LS_ERROR) << "InitializeAcl() failed";
    return false;
  }
  ...
  if (!::AddAccessAllowedAce(acl,
                             ...
                             token_user->User.Sid)) {
    LOG(LS_ERROR) << "AddAccessAllowedAce() failed";
    return false;
  }
  ...
  if (!::InitializeSecurityDescriptor(&security_descriptor,
    ...
    LOG(LS_ERROR) << "InitializeSecurityDescriptor() failed";
    return false;
  }
  ...
  if (!::SetSecurityDescriptorDacl(&security_descriptor,
                                   ...
                                   acl,
                                   FALSE)) {
    LOG(LS_ERROR) << "SetSecurityDescriptorDacl() failed";
    return false;
  }
  ...
  if (INVALID_HANDLE_VALUE == handle) {
    LOG(LS_ERROR) << "CreateFile() failed";
    return false;
  }
  if (!::CloseHandle(handle)) {
    LOG(LS_ERROR) << "CloseFile() failed";
    // Continue.
  }
  return true;
}
...
Pathname Win32Filesystem::GetCurrentDirectory() {
  ...
  do {
    ...
    if (needed == 0) {
      // Error.
      LOG(LS_ERROR) << "::GetCurrentDirectory() failed";
