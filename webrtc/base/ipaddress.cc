...
std::string IPAddress::ToString() const {
  ...
  if (!dii_rtc::inet_ntop(family_, src, buf, sizeof(buf))) {
    return std::string();
  }
...
std::string IPAddress::ToSensitiveString() const {
  ...
  switch (family_) {
    ...
    case AF_INET6: {
      ...
      size_t len =
          dii_rtc::sprintfn(&(result[0]), result.size(), "%x:%x:%x:x:x:x:x:x",
...
bool IPFromString(const std::string& str, IPAddress* out) {
  ...
  in_addr addr;
  if (dii_rtc::inet_pton(AF_INET, str.c_str(), &addr) == 0) {
    in6_addr addr6;
    if (dii_rtc::inet_pton(AF_INET6, str.c_str(), &addr6) == 0) {
      *out = IPAddress();
      return false;
    }
    *out = IPAddress(addr6);
...
IPAddress GetLoopbackIP(int family) {
  if (family == AF_INET) {
    return dii_rtc::IPAddress(INADDR_LOOPBACK);
  }
  if (family == AF_INET6) {
    return dii_rtc::IPAddress(in6addr_loopback);
  }
  return dii_rtc::IPAddress();
}

IPAddress GetAnyIP(int family) {
  if (family == AF_INET) {
    return dii_rtc::IPAddress(INADDR_ANY);
  }
  if (family == AF_INET6) {
    return dii_rtc::IPAddress(in6addr_any);
  }
  return dii_rtc::IPAddress();
}
