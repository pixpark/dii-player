...
#define WAIT(ex, timeout)                                       \
  for (int64_t start = dii_rtc::SystemTimeMillis();                 \
       !(ex) && dii_rtc::SystemTimeMillis() < start + (timeout);) { \
    dii_rtc::Thread::Current()->ProcessMessages(0);                 \
    dii_rtc::Thread::Current()->SleepMs(1);                         \
  }
...
#define WAIT_(ex, timeout, res)                                   \
  do {                                                            \
    int64_t start = dii_rtc::SystemTimeMillis();                      \
    res = (ex);                                                   \
    while (!res && dii_rtc::SystemTimeMillis() < start + (timeout)) { \
      dii_rtc::Thread::Current()->ProcessMessages(0);                 \
      dii_rtc::Thread::Current()->SleepMs(1);                         \
      res = (ex);                                                 \
    }                                                             \
...
#define SIMULATED_WAIT(ex, timeout, clock)                  \
  for (int64_t start = dii_rtc::TimeMillis();                   \
       !(ex) && dii_rtc::TimeMillis() < start + (timeout);) {   \
    clock.AdvanceTime(dii_rtc::TimeDelta::FromMilliseconds(1)); \
  }
...
#define SIMULATED_WAIT_(ex, timeout, res, clock)              \
  do {                                                        \
    ...
    res = (ex);                                               \
    while (!res && dii_rtc::TimeMillis() < start + (timeout)) {   \
      clock.AdvanceTime(dii_rtc::TimeDelta::FromMilliseconds(1)); \
      res = (ex);                                             \
    }                                                         \
  } while (0)
