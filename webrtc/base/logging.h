...
class LogSink {
 public:
  LogSink() {}
  virtual ~LogSink() {}
  virtual void OnLogMessage(int code, const std::string& message) = 0;
};

class LogMessage {
 public:
  LogMessage(const char* file, int line, LoggingSeverity sev,
             int stream_id = -1, int code = 0, int err = 0,
  ...
  static void LogToConsole(bool log_to_stderr);
 ...
 private:
  ...
  static bool thread_, timestamp_, log_tag_;
...
#define LOG_SEVERITY_PRECONDITION(sev) \
  !(dii_rtc::LogMessage::Loggable(sev)) \
    ? (void) 0 \
    : dii_rtc::LogMessageVoidify() &

#define LOG(sev) \
  LOG_SEVERITY_PRECONDITION(dii_rtc::sev) \
    dii_rtc::LogMessage(__FILE__, __LINE__, dii_rtc::sev).stream()

#define DII_LOG(sev, stream_id, code) \
 LOG_SEVERITY_PRECONDITION(dii_rtc::sev) \
  dii_rtc::LogMessage(__FILE__, __LINE__, dii_rtc::sev, stream_id, code).stream()

// The _V version is for when a variable is passed in.  It doesn't do the
// namespace concatination.
#define LOG_V(sev) \
  LOG_SEVERITY_PRECONDITION(sev) \
    dii_rtc::LogMessage(__FILE__, __LINE__, sev).stream()
...
#define LOG_CHECK_LEVEL(sev) \
  dii_rtc::LogCheckLevel(dii_rtc::sev)
#define LOG_CHECK_LEVEL_V(sev) \
  dii_rtc::LogCheckLevel(sev)
...
#define LOG_E(sev, ctx, err, ...) \
  LOG_SEVERITY_PRECONDITION(dii_rtc::sev) \
    dii_rtc::LogMessage(__FILE__, __LINE__, dii_rtc::sev, \
                          dii_rtc::ERRCTX_ ## ctx, err , ##__VA_ARGS__) \
        .stream()
...
#define LOG_TAG(sev, tag) \
  LOG_SEVERITY_PRECONDITION(sev) \
    dii_rtc::LogMessage(NULL, 0, sev, tag).stream()
