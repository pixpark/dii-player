...
struct DefaultDeleter {
  ...
  template <typename U> DefaultDeleter(const DefaultDeleter<U>& other) {
    ...
    static_assert(dii_rtc::is_convertible<U*, T*>::value,
...
//
// scoped_ptr<int, dii_rtc::FreeDeleter> foo_ptr(
...
struct ShouldAbortOnSelfReset {
  template <typename U>
  static dii_rtc::internal::NoType Test(const typename U::AllowSelfReset*);

  template <typename U>
  static dii_rtc::internal::YesType Test(...);

  static const bool value =
      sizeof(Test<T>(0)) == sizeof(dii_rtc::internal::YesType);
};
...
class scoped_ptr_impl {
 public:
  ...
  scoped_ptr_impl(scoped_ptr_impl<U, V>* other)
    ...
    // emulation to have dii_rtc::subtle::move() and dii_rtc::subtle::forward()
...
// types.
template <class T, class D = dii_rtc::DefaultDeleter<T> >
class scoped_ptr {

  // TODO(ajm): If we ever import RefCountedBase, this check needs to be
  // enabled.
  //static_assert(dii_rtc::internal::IsNotRefCounted<T>::value,
 ...
 public:
  ...
  scoped_ptr(scoped_ptr<U, V>&& other)
      : impl_(&other.impl_) {
    static_assert(!dii_rtc::is_array<U>::value, "U cannot be an array");
  }
 ...
 private:
  typedef dii_rtc::internal::scoped_ptr_impl<element_type, deleter_type>
      scoped_ptr::*Testable;
 ...
 private:
  ...
  dii_rtc::internal::scoped_ptr_impl<element_type, deleter_type> impl_;
...
template <class T, class D>
void swap(dii_rtc::scoped_ptr<T, D>& p1, dii_rtc::scoped_ptr<T, D>& p2) {
  p1.swap(p2);
}
...
template <class T, class D>
bool operator==(T* p1, const dii_rtc::scoped_ptr<T, D>& p2) {
  return p1 == p2.get();
}
...
template <typename T>
dii_rtc::scoped_ptr<T> rtc_make_scoped_ptr(T* ptr) {
  return dii_rtc::scoped_ptr<T>(ptr);
}
