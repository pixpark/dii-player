...
void NetworkManagerBase::GetAnyAddressNetworks(NetworkList* networks) {
  if (!ipv4_any_address_network_) {
    const dii_rtc::IPAddress ipv4_any_address(INADDR_ANY);
    ipv4_any_address_network_.reset(
        new dii_rtc::Network("any", "any", ipv4_any_address, 0));
  ...
  if (ipv6_enabled()) {
    if (!ipv6_any_address_network_) {
      const dii_rtc::IPAddress ipv6_any_address(in6addr_any);
      ipv6_any_address_network_.reset(
          new dii_rtc::Network("any", "any", ipv6_any_address, 0));
...
IPAddress BasicNetworkManager::QueryDefaultLocalAddress(int family) const {
  ...
  if (!socket) {
    LOG(LERROR) << "Socket creation failed";
    return IPAddress();
  }
