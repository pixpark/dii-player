...
class EventLogger final {
 public:
  ...
  void AddTraceEvent(const char* name,
                     ...
                     uint64_t timestamp,
                     int pid,
                     dii_rtc::PlatformThreadId thread_id) {
...
// https://docs.google.com/document/d/1CvAClvFfyA5R-PhYUmn5OOQtYMH4h6I0nSsKchNAySU/preview
  ...
  void Start(FILE* file, bool owned) {
    ...
    RTC_CHECK_EQ(0,
                 dii_rtc::AtomicOps::CompareAndSwap(&g_event_logging_active, 0, 1));
  ...
  void Stop() {
    ...
    if (dii_rtc::AtomicOps::CompareAndSwap(&g_event_logging_active, 1, 0) == 0)
      return;
 ...
 private:
  struct TraceEvent {
    ...
    uint64_t timestamp;
    int pid;
    dii_rtc::PlatformThreadId tid;
  };

  dii_rtc::CriticalSection crit_;
  std::vector<TraceEvent> trace_events_ GUARDED_BY(crit_);
  dii_rtc::PlatformThread logging_thread_;
  dii_rtc::Event shutdown_event_;
...
void InternalAddTraceEvent(char phase,
  ...
  if (dii_rtc::AtomicOps::AcquireLoad(&g_event_logging_active) == 0)
    return;

  g_event_logger->AddTraceEvent(name, category_enabled, phase,
                                dii_rtc::TimeMicros(), 1, dii_rtc::CurrentThreadId());
}

}  // namespace

void SetupInternalTracer() {
  RTC_CHECK(dii_rtc::AtomicOps::CompareAndSwapPtr(
                &g_event_logger, static_cast<EventLogger*>(nullptr),
                new EventLogger()) == nullptr);
  g_event_logger = new EventLogger();
  dii_media_kit::SetupEventTracer(InternalGetCategoryEnabled, InternalAddTraceEvent);
}
...
void ShutdownInternalTracer() {
  StopInternalCapture();
  EventLogger* old_logger = dii_rtc::AtomicOps::AcquireLoadPtr(&g_event_logger);
  RTC_DCHECK(old_logger);
  RTC_CHECK(dii_rtc::AtomicOps::CompareAndSwapPtr(
                &g_event_logger, old_logger,
                static_cast<EventLogger*>(nullptr)) == old_logger);
  delete old_logger;
  dii_media_kit::SetupEventTracer(nullptr, nullptr);
}
