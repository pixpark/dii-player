...
#define RTC_LAZY_STREAM(stream, condition)                                    \
  !(condition) ? static_cast<void>(0) : dii_rtc::FatalMessageVoidify() & (stream)
...
#define RTC_EAT_STREAM_PARAMETERS(condition) \
  (true ? true : !(condition))               \
      ? static_cast<void>(0)                 \
      : dii_rtc::FatalMessageVoidify() & dii_rtc::FatalMessage("", 0).stream()
...
#define RTC_CHECK(condition)                                      \
  RTC_LAZY_STREAM(dii_rtc::FatalMessage(__FILE__, __LINE__).stream(), \
                  !(condition))                                   \
...
#define RTC_CHECK_OP(name, op, val1, val2)                                 \
  if (std::string* _result =                                               \
          dii_rtc::Check##name##Impl((val1), (val2), #val1 " " #op " " #val2)) \
    dii_rtc::FatalMessage(__FILE__, __LINE__, _result).stream()
...
#define DEFINE_RTC_CHECK_OP_IMPL(name, op)                                   \
  ...
  inline std::string* Check##name##Impl(const t1& v1, const t2& v2,          \
    ...
    if (v1 op v2)                                                            \
      ...
      return dii_rtc::MakeCheckOpString(v1, v2, names);                          \
  }                                                                          \
...
#define FATAL() dii_rtc::FatalMessage(__FILE__, __LINE__).stream()
