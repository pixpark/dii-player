...
class NetworkManagerBase : public NetworkManager {
 ...
 private:
  ...
  Network* GetNetworkFromAddress(const dii_rtc::IPAddress& ip) const;
  ...
  std::unique_ptr<dii_rtc::Network> ipv4_any_address_network_;
  std::unique_ptr<dii_rtc::Network> ipv6_any_address_network_;
...
class Network {
 public:
  ...
  uint16_t GetCost() const {
    switch (type_) {
      case dii_rtc::ADAPTER_TYPE_ETHERNET:
      case dii_rtc::ADAPTER_TYPE_LOOPBACK:
        return kNetworkCostMin;
      case dii_rtc::ADAPTER_TYPE_WIFI:
      case dii_rtc::ADAPTER_TYPE_VPN:
        return kNetworkCostLow;
      case dii_rtc::ADAPTER_TYPE_CELLULAR:
