...
#if defined(WEBRTC_POSIX) && !defined(WEBRTC_MAC) && !defined(__native_client__) && !defined(WEBRTC_IOS)
...
int64_t GetSocketRecvTimestamp(int socket) {
  ...
  if (ret != 0)
    return -1;
  int64_t timestamp =
      dii_rtc::kNumMicrosecsPerSec * static_cast<int64_t>(tv_ioctl.tv_sec) +
...
std::unique_ptr<SocketServer> SocketServer::CreateDefault() {
  ...
  return std::unique_ptr<SocketServer>(new dii_rtc::NullSocketServer);
#else
  return std::unique_ptr<SocketServer>(new dii_rtc::PhysicalSocketServer);
#endif
}
  ...
  switch (opt) {
    case OPT_DONTFRAGMENT:
      ...
      *sopt = IP_DONTFRAGMENT;
      break;
#elif defined(WEBRTC_MAC) || defined(WEBRTC_IOS) || defined(BSD) || defined(__native_client__)
  ...
  if (res > 0) {
    ...
    switch (errno) {
      ...
      default:
        ...
        LOG(LS_WARNING) << "Assuming benign blocking error";
        return false;
    }
  }
}
...
class PosixSignalHandler {
 ...
 private:
  PosixSignalHandler() {
    if (pipe(afd_) < 0) {
      LOG(LS_ERROR) << "pipe failed";
      return;
    }
    if (fcntl(afd_[0], F_SETFL, O_NONBLOCK) < 0) {
      LOG(LS_WARNING) << "fcntl #1 failed";
    }
    if (fcntl(afd_[1], F_SETFL, O_NONBLOCK) < 0) {
      LOG(LS_WARNING) << "fcntl #2 failed";
    }
