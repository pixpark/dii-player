...
bool Worker::StartWork() {
  dii_rtc::Thread *me = dii_rtc::Thread::Current();
  if (worker_thread_) {
...
bool Worker::StopWork() {
  if (!worker_thread_) {
    ...
    return true;
  } else if (worker_thread_ != dii_rtc::Thread::Current()) {
...
void Worker::OnMessage(dii_rtc::Message *msg) {
  ASSERT(msg->message_id == MSG_HAVEWORK);
  ASSERT(worker_thread_ == dii_rtc::Thread::Current());
  OnHaveWork();
}
