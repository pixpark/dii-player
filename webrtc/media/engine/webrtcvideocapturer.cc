...
struct kVideoFourCCEntry {
  uint32_t fourcc;
  dii_media_kit::RawVideoType webrtc_type;
};

// This indicates our format preferences and defines a mapping between
// dii_media_kit::RawVideoType (from video_capture_defines.h) to our FOURCCs.
static kVideoFourCCEntry kSupportedFourCCs[] = {
  { FOURCC_I420, dii_media_kit::kVideoI420 },   // 12 bpp, no conversion.
  { FOURCC_YV12, dii_media_kit::kVideoYV12 },   // 12 bpp, no conversion.
  { FOURCC_YUY2, dii_media_kit::kVideoYUY2 },   // 16 bpp, fast conversion.
  { FOURCC_UYVY, dii_media_kit::kVideoUYVY },   // 16 bpp, fast conversion.
  { FOURCC_NV12, dii_media_kit::kVideoNV12 },   // 12 bpp, fast conversion.
  { FOURCC_NV21, dii_media_kit::kVideoNV21 },   // 12 bpp, fast conversion.
  { FOURCC_MJPG, dii_media_kit::kVideoMJPEG },  // compressed, slow conversion.
  { FOURCC_ARGB, dii_media_kit::kVideoARGB },   // 32 bpp, slow conversion.
  { FOURCC_24BG, dii_media_kit::kVideoRGB24 },  // 24 bpp, slow conversion.
};

class WebRtcVcmFactory : public WebRtcVcmFactoryInterface {
 public:
  virtual dii_rtc::scoped_refptr<dii_media_kit::VideoCaptureModule> Create(
      int id,
      const char* device) {
    return dii_media_kit::VideoCaptureFactory::Create(id, device);
  }
  virtual dii_media_kit::VideoCaptureModule::DeviceInfo* CreateDeviceInfo(int id) {
    return dii_media_kit::VideoCaptureFactory::CreateDeviceInfo(id);
  }
  virtual void DestroyDeviceInfo(dii_media_kit::VideoCaptureModule::DeviceInfo* info) {
    delete info;
  }
};

static bool CapabilityToFormat(const dii_media_kit::VideoCaptureCapability& cap,
                               VideoFormat* format) {
  uint32_t fourcc = 0;
...
static bool FormatToCapability(const VideoFormat& format,
                               dii_media_kit::VideoCaptureCapability* cap) {
  dii_media_kit::RawVideoType webrtc_type = dii_media_kit::kVideoUnknown;
  for (size_t i = 0; i < arraysize(kSupportedFourCCs); ++i) {
  ...
  if (webrtc_type == dii_media_kit::kVideoUnknown) {
    return false;
  }
  ...
  cap->codecType = dii_media_kit::kVideoCodecUnknown;
...
bool WebRtcVideoCapturer::Init(const Device& device) {
  ...
  dii_media_kit::VideoCaptureModule::DeviceInfo* info = factory_->CreateDeviceInfo(0);
  if (!info) {
    return false;
  }
...
bool WebRtcVideoCapturer::Init(
    const dii_rtc::scoped_refptr<dii_media_kit::VideoCaptureModule>& module) {
  RTC_DCHECK(!start_thread_);
  if (module_) {
    LOG(LS_ERROR) << "The capturer is already initialized";
    return false;
...
void WebRtcVideoCapturer::OnSinkWantsChanged(const dii_rtc::VideoSinkWants& wants) {
  ...
  const std::string group_name =
      dii_media_kit::field_trial::FindFullName("WebRTC-CVO");
...
CaptureState WebRtcVideoCapturer::Start(const VideoFormat& capture_format) {
  ...
  start_thread_ = dii_rtc::Thread::Current();
  RTC_DCHECK(!async_invoker_);
  async_invoker_.reset(new dii_rtc::AsyncInvoker());
  captured_frames_ = 0;
  ...
  int64_t start = dii_rtc::TimeMillis();
...
void WebRtcVideoCapturer::OnIncomingCapturedFrame(
    const int32_t id,
    const dii_media_kit::VideoFrame& sample) {
  ...
  if (start_thread_->IsCurrent()) {
    ...
    async_invoker_->AsyncInvoke<void>(
        ...
        // of sample.
        dii_rtc::Bind(&WebRtcVideoCapturer::SignalFrameCapturedOnStartThread, this,
                  sample));
  }
}
...
void WebRtcVideoCapturer::SignalFrameCapturedOnStartThread(
  ...
  // to one block for it.
  size_t length =
      dii_media_kit::CalcBufferSize(dii_media_kit::kI420, frame.width(), frame.height());
  ...
  dii_media_kit::ExtractBuffer(frame, length, &capture_buffer_[0]);
  WebRtcCapturedFrame webrtc_frame(frame, &capture_buffer_[0], length);
  SignalFrameCaptured(this, &webrtc_frame);
}

// WebRtcCapturedFrame
WebRtcCapturedFrame::WebRtcCapturedFrame(const dii_media_kit::VideoFrame& sample,
                                         void* buffer,
                                         size_t length) {
  width = sample.width();
  ...
  time_stamp = sample.render_time_ms() * dii_rtc::kNumNanosecsPerMillisec;
  data_size = dii_rtc::checked_cast<uint32_t>(length);
  data = buffer;
