...
class WebRtcVcmFactoryInterface {
 public:
  ...
  virtual dii_rtc::scoped_refptr<dii_media_kit::VideoCaptureModule> Create(
      int id,
      const char* device) = 0;
  virtual dii_media_kit::VideoCaptureModule::DeviceInfo* CreateDeviceInfo(int id) = 0;
  virtual void DestroyDeviceInfo(
      dii_media_kit::VideoCaptureModule::DeviceInfo* info) = 0;
};

// WebRTC-based implementation of VideoCapturer.
class WebRtcVideoCapturer : public VideoCapturer,
                            public dii_media_kit::VideoCaptureDataCallback {
 public:
  WebRtcVideoCapturer();
  ...
  bool Init(const dii_rtc::scoped_refptr<dii_media_kit::VideoCaptureModule>& module);
 ...
 protected:
  void OnSinkWantsChanged(const dii_rtc::VideoSinkWants& wants) override;
  // Override virtual methods of the parent class VideoCapturer.
 ...
 private:
  ...
  void OnIncomingCapturedFrame(const int32_t id,
                               const dii_media_kit::VideoFrame& frame) override;
  ...
  void SignalFrameCapturedOnStartThread(const dii_media_kit::VideoFrame& frame);

  std::unique_ptr<WebRtcVcmFactoryInterface> factory_;
  dii_rtc::scoped_refptr<dii_media_kit::VideoCaptureModule> module_;
  int captured_frames_;
  std::vector<uint8_t> capture_buffer_;
  dii_rtc::Thread* start_thread_;  // Set in Start(), unset in Stop();

  std::unique_ptr<dii_rtc::AsyncInvoker> async_invoker_;
};

struct WebRtcCapturedFrame : public CapturedFrame {
 public:
  WebRtcCapturedFrame(const dii_media_kit::VideoFrame& frame,
                      void* buffer,
                      size_t length);
};
