...
bool VideoAdapter::KeepFrame(int64_t in_timestamp_ns) {
  dii_rtc::CritScope cs(&critical_section_);
  ...
  next_frame_timestamp_ns_ =
      dii_rtc::Optional<int64_t>(in_timestamp_ns + requested_format_->interval / 2);
  return true;
}
...
void VideoAdapter::OnOutputFormatRequest(const VideoFormat& format) {
  dii_rtc::CritScope cs(&critical_section_);
  requested_format_ = dii_rtc::Optional<VideoFormat>(format);
  next_frame_timestamp_ns_ = dii_rtc::Optional<int64_t>();
}

void VideoAdapter::OnResolutionRequest(
    dii_rtc::Optional<int> max_pixel_count,
