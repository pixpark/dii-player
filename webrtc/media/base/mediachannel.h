...
template <class T>
static std::string ToStringIfSet(const char* key, const dii_rtc::Optional<T>& val) {
  std::string str;
  if (val) {
    str = key;
    str += ": ";
    str += val ? dii_rtc::ToString(*val) : "";
    str += ", ";
  }
  return str;
}
...
struct AudioOptions {
  ...
  dii_rtc::Optional<bool> echo_cancellation;
  // Audio processing to adjust the sensitivity of the local mic dynamically.
  dii_rtc::Optional<bool> auto_gain_control;
  // Audio processing to filter out background noise.
  dii_rtc::Optional<bool> noise_suppression;
  // Audio processing to remove background noise of lower frequencies.
  dii_rtc::Optional<bool> highpass_filter;
  // Audio processing to swap the left and right channels.
  dii_rtc::Optional<bool> stereo_swapping;
  // Audio receiver jitter buffer (NetEq) max capacity in number of packets.
  dii_rtc::Optional<int> audio_jitter_buffer_max_packets;
  // Audio receiver jitter buffer (NetEq) fast accelerate mode.
  dii_rtc::Optional<bool> audio_jitter_buffer_fast_accelerate;
  // Audio processing to detect typing.
  dii_rtc::Optional<bool> typing_detection;
  dii_rtc::Optional<bool> aecm_generate_comfort_noise;
  dii_rtc::Optional<int> adjust_agc_delta;
  dii_rtc::Optional<bool> experimental_agc;
  dii_rtc::Optional<bool> extended_filter_aec;
  dii_rtc::Optional<bool> delay_agnostic_aec;
  dii_rtc::Optional<bool> experimental_ns;
  dii_rtc::Optional<bool> intelligibility_enhancer;
  // Note that tx_agc_* only applies to non-experimental AGC.
  dii_rtc::Optional<uint16_t> tx_agc_target_dbov;
  dii_rtc::Optional<uint16_t> tx_agc_digital_compression_gain;
  dii_rtc::Optional<bool> tx_agc_limiter;
  dii_rtc::Optional<uint32_t> recording_sample_rate;
  dii_rtc::Optional<uint32_t> playout_sample_rate;
  // Enable combined audio+bandwidth BWE.
  // TODO(pthatcher): This flag is set from the
  // "googCombinedAudioVideoBwe", but not used anywhere. So delete it,
  // and check if any other AudioOptions members are unused.
  dii_rtc::Optional<bool> combined_audio_video_bwe;

 private:
  template <typename T>
  static void SetFrom(dii_rtc::Optional<T>* s, const dii_rtc::Optional<T>& o) {
    if (o) {
      *s = o;
    }
  }
...
struct VideoOptions {
  ...
  dii_rtc::Optional<bool> video_noise_reduction;
  ...
  dii_rtc::Optional<int> screencast_min_bitrate_kbps;
  ...
  dii_rtc::Optional<bool> is_screencast;
...
class MediaChannel : public sigslot::has_slots<> {
 public:
  class NetworkInterface {
   public:
    ...
    virtual bool SendPacket(dii_rtc::CopyOnWriteBuffer* packet,
                            const dii_rtc::PacketOptions& options) = 0;
    virtual bool SendRtcp(dii_rtc::CopyOnWriteBuffer* packet,
                          const dii_rtc::PacketOptions& options) = 0;
    virtual int SetOption(SocketType type, dii_rtc::Socket::Option opt,
                          int option) = 0;
  ...
  virtual void SetInterface(NetworkInterface *iface) {
    dii_rtc::CritScope cs(&network_interface_crit_);
    network_interface_ = iface;
    SetDscp(enable_dscp_ ? PreferredDscp() : dii_rtc::DSCP_DEFAULT);
  }
  virtual dii_rtc::DiffServCodePoint PreferredDscp() const {
    return dii_rtc::DSCP_DEFAULT;
  }
  // Called when a RTP packet is received.
  virtual void OnPacketReceived(dii_rtc::CopyOnWriteBuffer* packet,
                                const dii_rtc::PacketTime& packet_time) = 0;
  // Called when a RTCP packet is received.
  virtual void OnRtcpReceived(dii_rtc::CopyOnWriteBuffer* packet,
                              const dii_rtc::PacketTime& packet_time) = 0;
  ...
  virtual void OnNetworkRouteChanged(
      ...
      const dii_rtc::NetworkRoute& network_route) = 0;
  ...
  bool SendPacket(dii_rtc::CopyOnWriteBuffer* packet,
                  const dii_rtc::PacketOptions& options) {
    return DoSendPacket(packet, false, options);
  }

  bool SendRtcp(dii_rtc::CopyOnWriteBuffer* packet,
                const dii_rtc::PacketOptions& options) {
    return DoSendPacket(packet, true, options);
  }

  int SetOption(NetworkInterface::SocketType type,
                dii_rtc::Socket::Option opt,
                int option) {
    dii_rtc::CritScope cs(&network_interface_crit_);
 ...
 private:
  ...
  int SetDscp(dii_rtc::DiffServCodePoint value) {
    int ret;
    ret = SetOption(NetworkInterface::ST_RTP,
                    dii_rtc::Socket::OPT_DSCP,
                    value);
    if (ret == 0) {
  ...
  bool DoSendPacket(dii_rtc::CopyOnWriteBuffer* packet,
                    bool rtcp,
                    const dii_rtc::PacketOptions& options) {
    dii_rtc::CritScope cs(&network_interface_crit_);
    if (!network_interface_)
  ...
  dii_rtc::CriticalSection network_interface_crit_;
...
struct RtpParameters {
  ...
  std::vector<dii_media_kit::RtpExtension> extensions;
...
class VoiceMediaChannel : public MediaChannel {
 public:
  ...
  virtual dii_media_kit::RtpParameters GetRtpSendParameters(uint32_t ssrc) const = 0;
  virtual bool SetRtpSendParameters(
      uint32_t ssrc,
      const dii_media_kit::RtpParameters& parameters) = 0;
  virtual dii_media_kit::RtpParameters GetRtpReceiveParameters(
  ...
  virtual void SetRawAudioSink(
      uint32_t ssrc,
      std::unique_ptr<dii_media_kit::AudioSinkInterface> sink) = 0;
};
...
class VideoMediaChannel : public MediaChannel {
 public:
  ...
  virtual bool SetVideoSend(
      ...
      dii_rtc::VideoSourceInterface<cricket::VideoFrame>* source) = 0;
  // Sets the sink object to be used for the specified stream.
  // If SSRC is 0, the renderer is used for the 'default' stream.
  virtual bool SetSink(uint32_t ssrc,
                       dii_rtc::VideoSinkInterface<cricket::VideoFrame>* sink) = 0;
  // Gets quality stats for the channel.
...
class DataMediaChannel : public MediaChannel {
 public:
  ...
  virtual void OnNetworkRouteChanged(const std::string& transport_name,
                                     const dii_rtc::NetworkRoute& network_route) {}

  virtual bool SendData(
      const SendDataParams& params,
      const dii_rtc::CopyOnWriteBuffer& payload,
