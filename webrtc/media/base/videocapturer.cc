...
bool VideoCapturer::GetInputSize(int* width, int* height) {
  dii_rtc::CritScope cs(&frame_stats_crit_);
...
void VideoCapturer::RemoveSink(
    dii_rtc::VideoSinkInterface<cricket::VideoFrame>* sink) {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
...
void VideoCapturer::AddOrUpdateSink(
    dii_rtc::VideoSinkInterface<cricket::VideoFrame>* sink,
    const dii_rtc::VideoSinkWants& wants) {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  broadcaster_.AddOrUpdateSink(sink, wants);
  OnSinkWantsChanged(broadcaster_.wants());
}

void VideoCapturer::OnSinkWantsChanged(const dii_rtc::VideoSinkWants& wants) {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
...
bool VideoCapturer::AdaptFrame(int width,
  ...
  if (enable_video_adapter_ && !IsScreencast()) {
    if (!video_adapter_.AdaptFrameResolution(
            width, height, camera_time_us * dii_rtc::kNumNanosecsPerMicrosec,
...
void VideoCapturer::OnFrameCaptured(VideoCapturer*,
  ...
  if (!AdaptFrame(captured_frame->width, captured_frame->height,
                  captured_frame->time_stamp / dii_rtc::kNumNanosecsPerMicrosec,
                  0,
