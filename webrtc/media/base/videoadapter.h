...
class VideoAdapter {
 public:
  ...
  void OnResolutionRequest(dii_rtc::Optional<int> max_pixel_count,
                           dii_rtc::Optional<int> max_pixel_count_step_up);
 ...
 private:
  ...
  dii_rtc::Optional<int64_t> next_frame_timestamp_ns_ GUARDED_BY(critical_section_);

  // Max number of pixels requested via calls to OnOutputFormatRequest,
  // OnResolutionRequest respectively.
  // The adapted output format is the minimum of these.
  dii_rtc::Optional<VideoFormat> requested_format_ GUARDED_BY(critical_section_);
  ...
  dii_rtc::CriticalSection critical_section_;
