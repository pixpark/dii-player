...
class ScreencastId {
 public:
  ...
  explicit ScreencastId(const dii_rtc::WindowId& id)
      : type_(WINDOW), window_(id) {
  }
  explicit ScreencastId(const dii_rtc::DesktopId& id)
      : type_(DESKTOP), desktop_(id) {
  }

  Type type() const { return type_; }
  const dii_rtc::WindowId& window() const { return window_; }
  const dii_rtc::DesktopId& desktop() const { return desktop_; }
 ...
 private:
  Type type_;
  dii_rtc::WindowId window_;
  dii_rtc::DesktopId desktop_;
