...
bool Codec::GetParam(const std::string& name, int* out) const {
  ...
  if (iter == params.end())
    return false;
  return dii_rtc::FromString(iter->second, out);
}
...
void Codec::SetParam(const std::string& name, int value)  {
  params[name] = dii_rtc::ToString(value);
}
...
dii_media_kit::RtpCodecParameters Codec::ToCodecParameters() const {
  dii_media_kit::RtpCodecParameters codec_params;
...
dii_media_kit::RtpCodecParameters AudioCodec::ToCodecParameters() const {
  dii_media_kit::RtpCodecParameters codec_params = Codec::ToCodecParameters();
