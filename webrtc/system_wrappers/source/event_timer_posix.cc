...
dii_rtc::PlatformThread* EventTimerPosix::CreateThread() {
  const char* kThreadName = "WebRtc_event_timer_thread";
  return new dii_rtc::PlatformThread(Run, this, kThreadName);
}
...
bool EventTimerPosix::StartTimer(bool periodic, unsigned long time_ms) {
  ...
  time_ms_ = time_ms;
  timer_thread_->Start();
  timer_thread_->SetPriority(dii_rtc::kRealtimePriority);
  pthread_mutex_unlock(&mutex_);
