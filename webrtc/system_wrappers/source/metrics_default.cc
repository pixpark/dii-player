...
void CreateMap() {
  RtcHistogramMap* map = dii_rtc::AtomicOps::AcquireLoadPtr(&g_rtc_histogram_map);
  if (map == nullptr) {
    RtcHistogramMap* new_map = new RtcHistogramMap();
    RtcHistogramMap* old_map = dii_rtc::AtomicOps::CompareAndSwapPtr(
...
#if RTC_DCHECK_IS_ON
  dii_rtc::AtomicOps::ReleaseStore(&g_rtc_histogram_called, 1);
#endif
...
#if RTC_DCHECK_IS_ON
  RTC_DCHECK_EQ(0, dii_rtc::AtomicOps::AcquireLoad(&g_rtc_histogram_called));
#endif
  CreateMap();
}
