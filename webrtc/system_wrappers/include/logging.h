...
#define RESTRICT_LOGGING_PRECONDITION(sev)  \
  sev < dii_media_kit::LS_INFO ? (void) 0 :
#else
#define RESTRICT_LOGGING_PRECONDITION(sev)
#endif

#define LOG_SEVERITY_PRECONDITION(sev) \
  RESTRICT_LOGGING_PRECONDITION(sev) !(dii_media_kit::LogMessage::Loggable(sev)) \
    ? (void) 0 \
    : dii_media_kit::LogMessageVoidify() &

#define LOG(sev) \
  LOG_SEVERITY_PRECONDITION(dii_media_kit::sev) \
    dii_media_kit::LogMessage(__FILE__, __LINE__, dii_media_kit::sev).stream()

// The _V version is for when a variable is passed in.  It doesn't do the
// namespace concatination.
#define LOG_V(sev) \
  LOG_SEVERITY_PRECONDITION(sev) \
    dii_media_kit::LogMessage(__FILE__, __LINE__, sev).stream()
