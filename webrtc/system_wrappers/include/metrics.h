...
//   Histogram* dii_media_kit::metrics::HistogramFactoryGetCounts(
//       const std::string& name, int sample, int min, int max,
//       int bucket_count);
//   Histogram* dii_media_kit::metrics::HistogramFactoryGetEnumeration(
//       const std::string& name, int sample, int boundary);
//   void dii_media_kit::metrics::HistogramAdd(
...
#define RTC_HISTOGRAM_COUNTS(name, sample, min, max, bucket_count) \
  RTC_HISTOGRAM_COMMON_BLOCK(name, sample, false, \
      dii_media_kit::metrics::HistogramFactoryGetCounts(name, min, max, bucket_count))
...
#define RTC_HISTOGRAM_ENUMERATION(name, sample, boundary) \
  RTC_HISTOGRAM_COMMON_BLOCK(name, sample, false, \
      dii_media_kit::metrics::HistogramFactoryGetEnumeration(name, boundary))
...
#define RTC_HISTOGRAM_COMMON_BLOCK(constant_name, sample, log, \
  ...
  do { \
    ...
    static dii_media_kit::metrics::Histogram* atomic_histogram_pointer = nullptr; \
    dii_media_kit::metrics::Histogram* histogram_pointer = \
        dii_rtc::AtomicOps::AcquireLoadPtr(&atomic_histogram_pointer); \
    if (!histogram_pointer) { \
      histogram_pointer = factory_get_invocation; \
      dii_media_kit::metrics::Histogram* prev_pointer = \
          dii_rtc::AtomicOps::CompareAndSwapPtr( \
              &atomic_histogram_pointer, \
              static_cast<dii_media_kit::metrics::Histogram*>(nullptr), \
              histogram_pointer); \
      RTC_DCHECK(prev_pointer == nullptr || \
                 prev_pointer == histogram_pointer); \
    } \
    dii_media_kit::metrics::HistogramAdd(histogram_pointer, constant_name, sample); \
  } while (0)
...
#define RTC_HISTOGRAM_COMMON_BLOCK_SLOW(name, sample, factory_get_invocation) \
  do { \
    dii_media_kit::metrics::Histogram* histogram_pointer = factory_get_invocation; \
    dii_media_kit::metrics::HistogramAdd(histogram_pointer, name, sample); \
  } while (0)
