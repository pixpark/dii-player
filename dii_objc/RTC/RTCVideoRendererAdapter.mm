/*
 *  Copyright 2015 The WebRTC project@yuntongxun authors. All Rights Reserved.
 *
...
@implementation RTCVideoRendererAdapter {
  std::unique_ptr<dii_media_kit::VideoRendererAdapter> _adapter;
}
...
- (instancetype)initWithNativeRenderer:(id<RTCVideoRenderer>)videoRenderer {
  ...
  if (self = [super init]) {
    ...
    _adapter.reset(new dii_media_kit::VideoRendererAdapter(self));
  }
  return self;
}

- (dii_rtc::VideoSinkInterface<cricket::VideoFrame> *)nativeVideoRenderer {
  return _adapter.get();
}
