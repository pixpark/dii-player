...
-(BOOL)updateDrawInRect:(CGRect)rect withFrame:(DiiRTCVideoFrame *)frame {
      
    if(_lastDrawnRect.size.width == rect.size.width &&
       _lastDrawnRect.size.height == rect.size.height &&
       frame.height == _lastDrawnFrame.height &&
       frame.width == _lastDrawnFrame.width) {
        return YES;
    }
    
    _lastDrawnRect.size.width = rect.size.width;
    _lastDrawnRect.size.height = rect.size.height;
    
    float disp_aspect_ratio = (float)rect.size.width/rect.size.height;
    float frame_aspect_ratio = (float)frame.width/frame.height;
    ...
    if(disp_aspect_ratio > frame_aspect_ratio) {
        GLfloat gl_x = (GLfloat)frame_aspect_ratio/disp_aspect_ratio;
        vec[0] = -gl_x;
        vec[4] = gl_x;
        vec[8] = gl_x;
        vec[12] = -gl_x;
    } else {
        GLfloat gl_y = (GLfloat)disp_aspect_ratio/frame_aspect_ratio;
        vec[1] = -gl_y;
        vec[5] = -gl_y;
        vec[9] = gl_y;
        vec[13] = gl_y;
...
#if TARGET_OS_IPHONE
- (BOOL)drawFrame:(DiiRTCVideoFrame *)frame drawInRect:(CGRect)rect {
#elif TARGET_OS_MAC
- (BOOL)drawFrame:(DiiRTCVideoFrame *)frame {
#endif
...
#if TARGET_OS_IPHONE
        || ![self updateDrawInRect:rect withFrame:frame]) {
#elif TARGET_OS_MAC
    ) {
#endif
      return NO;
...
- (BOOL)updateTextureSizesForFrame:(DiiRTCVideoFrame *)frame {
...
- (BOOL)updateTextureDataForFrame:(DiiRTCVideoFrame *)frame {
