...
/**
 * Implements VideoRenderer.Callbacks by displaying the video stream on a SurfaceView.
 ...
 * Interaction from C++ dii_rtc::VideoSinkInterface in renderFrame.
...
public class SurfaceViewRenderer extends SurfaceView
    ...
    private VideoRenderer.I420Frame currentFrame;
    ...
    public void init(EglBase.Context sharedContext, RendererCommon.RendererEvents rendererEvents,
        ...
        synchronized (handlerLock) {
            if (renderThreadHandler != null) {
                Logging.w(TAG, getResourceName() + "Already initialized");
                return;
            }
        ...
        runOnRenderThread(new Runnable() {
            ...
            public void run() {
                synchronized (layoutLock) {
                    if (isSurfaceCreated && (eglBase != null) && !eglBase.hasSurface()) {
    ...
    public void release() {
        ...
        synchronized (frameLock) {
            ...
            if (currentFrame != null) {
                VideoRenderer.renderFrameDone(currentFrame);
                currentFrame = null;
            }
        }
    ...
    private synchronized void callbackShotResult(Bitmap bitmap) {
        if (videoShotHelper != null) {
            videoShotHelper.onFrame(bitmap);
            videoShotHelper = null;
        }
    }
...
//            makeBlack();
        ...
        VideoRenderer.ViewportFrame vp_frame;
        synchronized (layoutLock) {
            final float[] rotatedSamplingMatrix =
                    RendererCommon.rotateTextureMatrix(frame.samplingMatrix, frame.rotationDegree);

            final float[] layoutMatrix = RendererCommon.getLayoutMatrix(
                    mirror, frameAspectRatio(), frameAspectRatio());
            texMatrix = RendererCommon.multiplyMatrices(rotatedSamplingMatrix, layoutMatrix);
            vp_frame = computViewportFrame(frameAspectRatio(), surfaceSize.x, surfaceSize.y);
        }
        ...
        try {
            ...
            if (frame.yuvFrame) {
                ...
                drawer.drawYuv(yuvTextures, texMatrix, frame.rotatedWidth(), frame.rotatedHeight(),
                        vp_frame.x, vp_frame.y, vp_frame.width, vp_frame.height);
            ...
            Logging.e(TAG, "Error while drawing frame Exception:" + e.toString());
    ...
    private VideoRenderer.ViewportFrame computViewportFrame(float frameAspectRatio, int dispWidth, int dispHeight) {
        int vp_x = 0;
        int vp_y = 0;
        int vp_width = dispWidth;
        int vp_height = dispHeight;
        float displayAspectRatio = (float) dispWidth / dispHeight;

        if (displayAspectRatio < frameAspectRatio()) {
            vp_height = (int) ((float) dispWidth / frameAspectRatio());
            vp_y = (dispHeight - vp_height) / 2;
        } else {
            vp_width = (int) ((float) dispHeight * frameAspectRatio());
            vp_x = (dispWidth - vp_width) / 2;
        }

        return new VideoRenderer.ViewportFrame(vp_x, vp_y, vp_width, vp_height);
    }
    ...
    // add by devzhaoyou, save frame with bitmap format
    public interface Helper {
        public void onFrame(Bitmap bmp);
    }

    Helper videoShotHelper = null;
    public void getBitmapFrame(final Helper helper) {
        if(helper == null) return;

        videoShotHelper = helper;
        synchronized (handlerLock) {
            if (renderThreadHandler == null) {
                callbackShotResult(null);
                return;
