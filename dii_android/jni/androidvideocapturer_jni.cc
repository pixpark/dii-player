...
AndroidVideoCapturerJni::AndroidVideoCapturerJni(JNIEnv* jni,
    ...
    : j_video_capturer_(jni, j_video_capturer),
      j_video_capturer_class_(jni, FindClass(jni, "org/dii/webrtc/VideoCapturer")),
      j_observer_class_(
          jni,
          FindClass(jni,
                    "org/dii/webrtc/VideoCapturer$NativeObserver")),
...
void AndroidVideoCapturerJni::Start(int width, int height, int framerate,
                                    dii_media_kit::AndroidVideoCapturer* capturer) {
  LOG(LS_INFO) << "AndroidVideoCapturerJni start";
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  {
    dii_rtc::CritScope cs(&capturer_lock_);
    RTC_CHECK(capturer_ == nullptr);
    RTC_CHECK(invoker_.get() == nullptr);
    capturer_ = capturer;
    invoker_.reset(new dii_rtc::GuardedAsyncInvoker());
  }
  ...
  jmethodID m = GetMethodID(
      ...
      "(IIILorg/dii/webrtc/SurfaceTextureHelper;Landroid/content/Context;"
      "Lorg/dii/webrtc/VideoCapturer$CapturerObserver;)V");
  jni()->CallVoidMethod(
...
void AndroidVideoCapturerJni::AsyncCapturerInvoke(
    const dii_rtc::Location& posted_from,
    void (dii_media_kit::AndroidVideoCapturer::*method)(Args...),
  ...
  invoker_->AsyncInvoke<void>(posted_from,
                              dii_rtc::Bind(method, capturer_, args...));
}
...
AndroidVideoCapturerJni::GetSupportedFormats() {
  ...
  jclass j_format_class =
      jni->FindClass("org/dii/webrtc/CameraEnumerationAndroid$CaptureFormat");
  jclass j_framerate_class = jni->FindClass(
      "org/dii/webrtc/CameraEnumerationAndroid$CaptureFormat$FramerateRange");
  ...
  jfieldID j_framerate_field = GetFieldID(
      ...
      "Lorg/dii/webrtc/CameraEnumerationAndroid$CaptureFormat$FramerateRange;");
...
void AndroidVideoCapturerJni::OnCapturerStarted(bool success) {
  ...
  AsyncCapturerInvoke(
      RTC_FROM_HERE, &dii_media_kit::AndroidVideoCapturer::OnCapturerStarted, success);
}
...
void AndroidVideoCapturerJni::OnMemoryBufferFrame(void* video_frame,
  ...
  if (!capturer_->AdaptFrame(width, height,
                             timestamp_ns / dii_rtc::kNumNanosecsPerMicrosec,
                             dii_rtc::TimeMicros(),
  ...
  dii_libyuv::NV12ToI420Rotate(
      ...
      crop_width, crop_height, static_cast<dii_libyuv::RotationMode>(
          capturer_->apply_rotation() ? rotation : 0));

  if (adapted_width != buffer->width() || adapted_height != buffer->height()) {
    dii_rtc::scoped_refptr<dii_media_kit::I420Buffer> scaled_buffer(
  ...
  capturer_->OnFrame(cricket::WebRtcVideoFrame(
                         ...
                         capturer_->apply_rotation()
                             ...
                             : static_cast<dii_media_kit::VideoRotation>(rotation),
                         translated_camera_time_us),
                     width, height);
}
...
void AndroidVideoCapturerJni::OnTextureFrame(int width,
  ...
  if (capturer_->apply_rotation()) {
    ...
    matrix.Rotate(static_cast<dii_media_kit::VideoRotation>(rotation));
  }
...
void AndroidVideoCapturerJni::OnOutputFormatRequest(int width,
  ...
  AsyncCapturerInvoke(RTC_FROM_HERE,
                      &dii_media_kit::AndroidVideoCapturer::OnOutputFormatRequest,
                      width, height, fps);
}
