...
using dii_rtc::Bind;
using dii_rtc::Thread;
using dii_rtc::ThreadManager;

using dii_media_kit::CodecSpecificInfo;
using dii_media_kit::EncodedImage;
using dii_media_kit::VideoFrame;
using dii_media_kit::RTPFragmentationHeader;
using dii_media_kit::VideoCodec;
using dii_media_kit::VideoCodecType;
using dii_media_kit::kVideoCodecH264;
using dii_media_kit::QualityScaler;
...
#define ALOGV(...)
#endif
#define ALOGD LOG_TAG(dii_rtc::LS_INFO, TAG_ENCODER)
#define ALOGW LOG_TAG(dii_rtc::LS_WARNING, TAG_ENCODER)
#define ALOGE LOG_TAG(dii_rtc::LS_ERROR, TAG_ENCODER)
...
// MediaCodecVideoEncoder is a dii_media_kit::VideoEncoder implementation that uses
// Android's MediaCodec SDK API behind the scenes to implement (hopefully)
// HW-backed video encode.  This C++ class is implemented as a very thin shim,
// delegating all of the interesting work to MediaCodecVideoEncoder.
// MediaCodecVideoEncoder is created, operated, and destroyed on a single
// thread, currently the libjingle Worker thread.
class MediaCodecVideoEncoder : public dii_media_kit::VideoEncoder,
 ...
 public:
  ...
  // dii_media_kit::VideoEncoder implementation.  Everything trampolines to
  // |codec_thread_| for execution.
  int32_t InitEncode(const dii_media_kit::VideoCodec* codec_settings,
                     int32_t /* number_of_cores */,
                     size_t /* max_payload_size */) override;
  int32_t Encode(const dii_media_kit::VideoFrame& input_image,
                 const dii_media_kit::CodecSpecificInfo* /* codec_specific_info */,
                 const std::vector<dii_media_kit::FrameType>* frame_types) override;
  int32_t RegisterEncodeCompleteCallback(
      dii_media_kit::EncodedImageCallback* callback) override;
  ...
  // dii_rtc::MessageHandler implementation.
 ...
 private:
  ...
  // Implementation of dii_media_kit::VideoEncoder methods above, all running on the
  ...
  bool MaybeReconfigureEncoderOnCodecThread(const dii_media_kit::VideoFrame& frame);
  int32_t EncodeOnCodecThread(
      const dii_media_kit::VideoFrame& input_image,
      const std::vector<dii_media_kit::FrameType>* frame_types,
      const int64_t frame_input_time_ms);
  bool EncodeByteBufferOnCodecThread(JNIEnv* jni,
      bool key_frame, const dii_media_kit::VideoFrame& frame, int input_buffer_index);
  bool EncodeTextureOnCodecThread(JNIEnv* jni,
      bool key_frame, const dii_media_kit::VideoFrame& frame);

  int32_t RegisterEncodeCompleteCallbackOnCodecThread(
      dii_media_kit::EncodedImageCallback* callback);
  ...
  dii_media_kit::EncodedImageCallback* callback_;

  // State that is constant for the lifetime of this object once the ctor
  // returns.
  std::unique_ptr<Thread>
      codec_thread_;  // Thread on which to operate MediaCodec.
  dii_rtc::ThreadChecker codec_thread_checker_;
  ...
  enum dii_libyuv::FourCC encoder_fourcc_;  // Encoder color space format.
  ...
  struct InputFrameInfo {
    ...
    const dii_media_kit::VideoRotation rotation;
  };
  ...
  int64_t output_render_time_ms_; // Last output frame render time from
                                  // |input_frame_infos_|.
  dii_media_kit::VideoRotation output_rotation_;  // Last output frame rotation from
                                           // |input_frame_infos_|.
  ...
  dii_media_kit::H264BitstreamParser h264_bitstream_parser_;

  // VP9 variables to populate codec specific structure.
  dii_media_kit::GofInfoVP9 gof_; // Contains each frame's temporal information for
  ...
  dii_media_kit::VideoCodecMode codec_mode_;
};
...
MediaCodecVideoEncoder::MediaCodecVideoEncoder(
    ...
    j_media_codec_video_encoder_class_(
        jni,
        FindClass(jni, "org/dii/webrtc/MediaCodecVideoEncoder")),
  ...
  jclass j_output_buffer_info_class =
      FindClass(jni, "org/dii/webrtc/MediaCodecVideoEncoder$OutputBufferInfo");
  j_init_encode_method_ = GetMethodID(
      jni,
      *j_media_codec_video_encoder_class_,
      "initEncode",
      "(Lorg/dii/webrtc/MediaCodecVideoEncoder$VideoCodecType;"
      "IIIILorg/dii/webrtc/EglBase14$Context;)Z");
  ...
  j_dequeue_output_buffer_method_ = GetMethodID(
      ...
      *j_media_codec_video_encoder_class_,
      "dequeueOutputBuffer",
      "()Lorg/dii/webrtc/MediaCodecVideoEncoder$OutputBufferInfo;");
...
int32_t MediaCodecVideoEncoder::InitEncode(
    const dii_media_kit::VideoCodec* codec_settings,
    int32_t /* number_of_cores */,
...
int32_t MediaCodecVideoEncoder::Encode(
    const dii_media_kit::VideoFrame& frame,
    const dii_media_kit::CodecSpecificInfo* /* codec_specific_info */,
    const std::vector<dii_media_kit::FrameType>* frame_types) {
  return codec_thread_->Invoke<int32_t>(
      RTC_FROM_HERE, Bind(&MediaCodecVideoEncoder::EncodeOnCodecThread, this,
                          frame, frame_types, dii_rtc::TimeMillis()));
}
...
void MediaCodecVideoEncoder::OnMessage(dii_rtc::Message* msg) {
...
int32_t MediaCodecVideoEncoder::InitEncodeOnCodecThread(
  ...
  stat_start_time_ms_ = dii_rtc::TimeMillis();
  current_frames_ = 0;
  current_bytes_ = 0;
  current_acc_qp_ = 0;
  ...
  gof_.SetGofInfoVP9(dii_media_kit::TemporalStructureMode::kTemporalStructureMode1);
  ...
  if (!use_surface) {
    ...
    switch (GetIntField(jni, *j_media_codec_video_encoder_,
      ...
      case COLOR_FormatYUV420Planar:
        encoder_fourcc_ = dii_libyuv::FOURCC_YU12;
        break;
      case COLOR_FormatYUV420SemiPlanar:
      case COLOR_QCOM_FormatYUV420SemiPlanar:
      case COLOR_QCOM_FORMATYUV420PackedSemiPlanar32m:
        encoder_fourcc_ = dii_libyuv::FOURCC_NV12;
        break;
      default:
...
int32_t MediaCodecVideoEncoder::EncodeOnCodecThread(
  ...
  if (codec_mode_ == dii_media_kit::kRealtimeVideo) {
    ++frames_received_since_last_key_;
    int64_t now_ms = dii_rtc::TimeMillis();
  ...
  if (drop_next_input_frame_) {
    ...
    drop_next_input_frame_ = false;
    current_timestamp_us_ += dii_rtc::kNumMicrosecsPerSec / last_set_fps_;
  ...
  dii_rtc::scoped_refptr<dii_media_kit::VideoFrameBuffer> input_buffer(
      frame.video_frame_buffer());
  if (scale_) {
    // Check framerate before spatial resolution change.
    quality_scaler_.OnEncodeFrame(frame.width(), frame.height());
    const dii_media_kit::QualityScaler::Resolution scaled_resolution =
  ...
  const bool key_frame =
      frame_types->front() != dii_media_kit::kVideoFrameDelta || send_key_frame;
  ...
  last_input_timestamp_ms_ =
      current_timestamp_us_ / dii_rtc::kNumMicrosecsPerMillisec;
...
bool MediaCodecVideoEncoder::EncodeByteBufferOnCodecThread(JNIEnv* jni,
  ...
  RTC_CHECK(!dii_libyuv::ConvertFromI420(
...
int32_t MediaCodecVideoEncoder::ReleaseOnCodecThread() {
  ...
  CHECK_EXCEPTION(jni);
  dii_rtc::MessageQueueManager::Clear(this);
  inited_ = false;
  use_surface_ = false;
...
bool MediaCodecVideoEncoder::DeliverPendingOutputs(JNIEnv* jni) {
  ...
  while (true) {
    ...
    last_output_timestamp_ms_ =
        ...
        dii_rtc::kNumMicrosecsPerMillisec;
    if (!input_frame_infos_.empty()) {
    ...
    if (callback_) {
      std::unique_ptr<dii_media_kit::EncodedImage> image(
          new dii_media_kit::EncodedImage(payload, payload_size, payload_size));
      ...
      image->_frameType =
          (key_frame ? dii_media_kit::kVideoFrameKey : dii_media_kit::kVideoFrameDelta);
      image->_completeFrame = true;
      image->adapt_reason_.quality_resolution_downscales =
          scale_ ? quality_scaler_.downscale_shift() : -1;

      dii_media_kit::CodecSpecificInfo info;
      memset(&info, 0, sizeof(info));
      info.codecType = codecType_;
      picture_id_ = (picture_id_ + 1) & 0x7FFF;

      // Generate a header describing a single fragment.
      dii_media_kit::RTPFragmentationHeader header;
    ...
    if (encoding_start_time_ms > 0) {
      frame_encoding_time_ms = dii_rtc::TimeMillis() - encoding_start_time_ms;
    }
    if (frames_encoded_ < kMaxEncodedLogFrames) {
      int current_latency =
...
void MediaCodecVideoEncoder::LogStatistics(bool force_log) {
  int statistic_time_ms = dii_rtc::TimeMillis() - stat_start_time_ms_;
...
MediaCodecVideoEncoderFactory::MediaCodecVideoEncoderFactory()
  ...
  JNIEnv* jni = AttachCurrentThreadIfNeeded();
  ScopedLocalRefFrame local_ref_frame(jni);
  jclass j_encoder_class = FindClass(jni, "org/dii/webrtc/MediaCodecVideoEncoder");
...
dii_media_kit::VideoEncoder* MediaCodecVideoEncoderFactory::CreateVideoEncoder(
    VideoCodecType type) {
...
void MediaCodecVideoEncoderFactory::DestroyVideoEncoder(
    dii_media_kit::VideoEncoder* encoder) {
